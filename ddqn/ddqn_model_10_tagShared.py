...
class ActorPickNewsQ():
    ...
    def __init__(self,tau,gamma,
        ...
        with tf.variable_scope(scope):
            ...
            with tf.variable_scope('actor_Q_net') as actor_scope:
                ...
                self.action_record_tmp,self.step_reward_tmp  =self.pick_news_drop_batch(candi_user_nfm=self.candi_user_nfm,
                ...
                self.action_choose, self.step_choose_reward_tmp = self.pick_news_drop_batch(candi_user_nfm=self.candi_user_nfm,
                                                                  candidates_tags=self.candidates_tags,
                                                                  feed_action=False,
                                                                  nfm_model=self.nfm_model)
    ...
    def pick_news_drop_batch(self, candi_user_nfm, candidates_tags, nfm_model,feed_action=False):
        ...
        def step(candi_user_nfm_trans, candidates_tags, history_newsTags_current,indices_mask_current,i):
            ...
            # news_probs_tmp=self.predict_out_in_conbined(candi_user_nfm_trans, candi_hist_nfm,i)
    ...
    def predict_action(self,states,sess=None):
        ...
        return sess.run([self.action_choose,self.step_choose_reward,self.value_choose_estimate],feed_in)
    ...
    def build_candi_hist_nfm(self,candi_tags,hist_tags,nfm_model,history_position_weights):
        ...
        candi_hist_nfm = nfm_model.build_nfm_candidates_innerHistory_batch(
    ...
    def predict_out_in_conbined(self, candi_user_current, newsTags_history, step):
        with tf.variable_scope(name_or_scope='predict_out_in_conbined', reuse=tf.AUTO_REUSE):
            # step_embeddings = nfm_model.position_embedding(step,
            #                                                10,
            #                                                hypes.tags_emb_size,
            #                                                scope="step_embedding")
            candi_shape = candi_user_current.get_shape().as_list()

            step_tile = tf.tile(tf.reshape(float(step), [1, 1, 1]), [candi_shape[0],
                                                                     candi_shape[1], 1], name='step_tile')
            in_overall = tf.concat([candi_user_current, newsTags_history, step_tile], 2)  # [b,200,128+128]
            # in_overall = tf.reshape(in_overall, [self.batch_size, 1, -1, hypes.tags_emb_size * 2])
            in_overall = tf.expand_dims(in_overall, 2)
