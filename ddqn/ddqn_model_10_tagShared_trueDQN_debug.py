...
class ActorPickNewsQ():
    ...
    def __init__(self,tau,gamma,
        ...
        with tf.variable_scope(scope):
            ...
            self.action_recommended = tf.placeholder(dtype=tf.int32, shape=[batch_size,hypes.num_news_action], name='action_in_record')
            ...
            with tf.variable_scope('actor_Q_net') as actor_scope:
                ...
                self.candi_user_nfm = self.nfm_model.build_nfm_candidates_user_outerHistory_batch(
                ...
                self.action_record_tmp,self.step_reward_tmp,self.probs_mat  =self.pick_news_drop_batch(candi_user_nfm=self.candi_user_nfm,
                ...
                self.action_choose, self.step_choose_reward_tmp, self.probs_mat_predicted= self.pick_news_drop_batch(candi_user_nfm=self.candi_user_nfm,
                                                                  candidates_tags=self.candidates_tags,
                                                                  feed_action=False,
                                                                  nfm_model=self.nfm_model)

                self.step_choose_reward = tf.transpose(self.step_choose_reward_tmp, [1, 0])
                self.action_choose=tf.transpose(self.action_choose,[1,0])
                self.value_choose_estimate = self.q_value_net(self.step_choose_reward)
            ...
            with tf.variable_scope('target_Q_net') as target_scope:
                ...
                _, self.target_step_reward,self.target_probs_mat= self.pick_news_drop_batch(candi_user_nfm=self.target_candi_user_nfm,
                                                                      candidates_tags=self.candidates_tags,
                                                                      feed_action=True,
                ...
                self.target_action, \
                self.target_action_reward,self.target_action_probs_mat = self.pick_news_drop_batch(candi_user_nfm=self.target_candi_user_nfm,
                                                                      candidates_tags=self.candidates_tags,
                                                                      feed_action=False,
                                                                      nfm_model=self.target_nfm_model)
                ...
                self.target_action_reward_tensor=tf.convert_to_tensor(self.target_action_reward,
            ...
            self.loss = tf.losses.mean_squared_error(self.y, self.step_reward)
            with tf.name_scope("weights_decay"):
                self.weight_decay = tf.add_n([hypes.weight_decay_rate * tf.nn.l2_loss(var) for var in self.network_params])
            ...
            for i in range(hypes.num_news_action):
                s = tf.summary.scalar('step_reward/train_action_probs_' + str(i),
                                      tf.reduce_sum(self.step_reward[:, i]) / batch_size)
                list_summary.append(s)

            for i in range(hypes.num_news_action):
                s = tf.summary.scalar('target_step_reward/target_action_probs_' + str(i),
                                      tf.reduce_sum(self.target_step_reward[:, i]) / batch_size)
                list_summary.append(s)
    ...
    def pick_news_drop_batch(self, candi_user_nfm, candidates_tags, nfm_model,feed_action=False):
        ...
        def step(candi_user_nfm_trans, candidates_tags, history_newsTags_current,indices_mask_current,i):
            ...
            res_step_rewards.append(news_probs)
        ...
        return res_positions, res_rewards,res_step_rewards
    ...
    def predict_action(self,states,sess=None):
        ...
        return sess.run([self.action_choose,self.step_choose_reward,self.value_choose_estimate,self.probs_mat_predicted],feed_in)
    ...
    def predict_target_value(self, states, action,sess=None):
        ...
        feed_in = {self.candidates_tags: candidates_tags_batch,
                   ...
                   self.history_positions: h_positions_batch,
                   self.candidates_mask: candidates_mask_batch}
        return sess.run([self.target_step_reward,self.target_probs_mat], feed_in)
    ...
    def predict_target(self, states,sess=None):
        ...
        return sess.run([self.target_action,
                         ...
                         self.target_action_probs_mat],feed_in)
