...
import Console
from ddqn import ddqn_model_10_tagShared as ddqn
from environment import environment_10_newsTags_tagShared as env
...
def train(sess, userEnv, actor_Q, saver, summary_writer, checkpoint_path, args,replay_buffer):
    ...
    while epoch < args.number_epoch:
        ...
        for t in itertools.count():
            ...
            if (replay_buffer.size() >= args.buffer_size*1 and add_step % (args.batch_size // 2) == 0):
                s_batch, a_batch, r_batch, s2_batch, t_batch = replay_buffer.sample_batch(args.batch_size)
                y_batch = []

                a,_,_ = actor_Q.predict_action(s2_batch)
                target_q_batch = actor_Q.predict_target_value(s2_batch, a)
                for i in range(args.batch_size):
                    if len(s2_batch[i].candidates_news) > 10:
                        y_batch.append(r_batch[i] + actor_Q.gamma * target_q_batch[i])
                    else:
                ...
                if(global_step % 10 ==0):
                    ...
                    _, step_choose_probs,value_choose_estimate = actor_Q.predict_action(s_batch)

                    build_summary(s_batch, s2_batch, y_batch,
                                  predicted_q_value,target_q_batch,
                                  target_probs,value_target_estimate,
                                  step_choose_probs,value_choose_estimate,
...
def build_summary(s_batch, s2_batch, y_batch,
                  predicted_q_value,target_q_batch,
                  target_probs,value_target_estimate,
                  step_choose_probs,value_choose_estimate,
                  summary_writer, global_step):
    ...
    value_target_estimate_ave = 0.
    value_choose_estimate_ave = 0.

    target_next_ave = 0.
    target_next_non_zero_ave = 0.
    target_next_zero_ave = 0.
    ...
    tq_c = 0
    tq_next_non_zero_c = 0
    tq_next_zero_c = 0
    for i, s in enumerate(s_batch):
        if s_batch[i].position == 1:
            target_ave += y_batch[i]
            q_value_ave += predicted_q_value[i]
            if len(s2_batch[i].candidates_news) > 10:
                target_next_non_zero_ave += target_q_batch[i]
                tq_next_non_zero_c += 1
            else:
                target_next_zero_ave += target_q_batch[i]
                tq_next_zero_c += 1
            value_target_estimate_ave += value_target_estimate[i]
            value_choose_estimate_ave += value_choose_estimate[i]
    ...
    if tq_c > 1:
        ...
        target_next_ave = (target_next_non_zero_ave + target_next_zero_ave) / tq_c
        value_target_estimate_ave = value_target_estimate_ave / tq_c
        value_choose_estimate_ave = value_choose_estimate_ave / tq_c
    ...
    if tq_next_non_zero_c > 1:
        target_next_non_zero_ave = target_next_non_zero_ave / tq_next_non_zero_c
    if tq_next_zero_c > 1:
        target_next_zero_ave = target_next_zero_ave / tq_next_zero_c

    summary_tq = tf.Summary()
    summary_tq.value.add(tag='value/target_ave', simple_value=target_ave)
    summary_tq.value.add(tag='value/q_value_ave', simple_value=q_value_ave)
    summary_tq.value.add(tag='value/target_next_ave', simple_value=target_next_ave)
    summary_tq.value.add(tag='value/value_target_estimate_ave', simple_value=value_target_estimate_ave)
    summary_tq.value.add(tag='value/value_choose_estimate_ave', simple_value=value_choose_estimate_ave)

    summary_tq.value.add(tag='value/target_next_non_zero_ave', simple_value=target_next_non_zero_ave)
    summary_tq.value.add(tag='value/target_next_zero_ave', simple_value=target_next_zero_ave)
...
def saveAsPb(sess, path):
    ...
    output_graph_def = graph_util.convert_variables_to_constants(
        sess, gd,
        output_node_names=["train/Actor_1/candidates_input", "train/Actor_1/user_input", "train/Actor_1/user_input_w",
                           "train/Actor_1/history_newsTags", "train/Actor_1/history_clicks",
                           "train/Actor_1/history_positions", "train/Actor_1/target_Q_net/transpose_2"],
        variable_names_whitelist=None)
        ...
        while True:
            ...
            if len(train_files) < 1:
                ...
                for file_num in range(len(train_files)):
                    ...
                    #dev
                    shell_path = "/data/bd-recommend/lizhenmao/transfer_log_tagShared_dev/updateVideoChannelModel.sh"
                    os.system(shell_path)
