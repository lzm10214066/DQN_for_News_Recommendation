...
from utility import replay_priority as replay
...
def train(sess, userEnv, actor_Q, actor_Q_test, saver, summary_writer, checkpoint_path, args,replay_buffer):
    ...
    while epoch < args.number_epoch:
        ...
        for t in itertools.count():
            ...
            if valid == True:
                vc += 1
                states_one_batch.append(state)
                actions_one_batch.append(action)
                rewards_one_batch.append(rewards)
                next_state_one_batch.append(next_state)
                dones_one_batch.append(done)

                if(len(states_one_batch) == args.batch_size):
                    y_one_batch = np.array(rewards_one_batch)
                    a, _, _ = actor_Q.predict_action(states_one_batch)
                    target_step_reward_one = actor_Q.predict_target_value(states_one_batch, actions_one_batch)
                    for i in range(hypes.num_news_action - 1):
                        y_one_batch[:, i] += actor_Q.gamma * target_step_reward_one[:, i + 1]

                    td_error = actor_Q.get_TD_error(states_one_batch, actions_one_batch, y_one_batch)[0]
                    for i in range(args.batch_size):
                        replay_buffer.add(
                            Transition(state=states_one_batch[i], action=actions_one_batch[i], reward=rewards_one_batch[i],
                                       next_state=next_state_one_batch[i], done=dones_one_batch[i]),TDerror=td_error[i])
                        add_step += 1

                    states_one_batch.clear()
                    actions_one_batch.clear()
                    rewards_one_batch.clear()
                    next_state_one_batch.clear()
                    dones_one_batch.clear()
            ...
            if (replay_buffer.size() >= args.buffer_size*args.start_rate and add_step % (args.batch_size) == 0):
                s_batch, a_batch, r_batch, s2_batch, t_batch = replay_buffer.batch(args.batch_size)

                y_batch = r_batch
                # t_s = time.time()
                a,_,_ = actor_Q.predict_action(s_batch)
                target_step_reward_batch = actor_Q.predict_target_value(s_batch, a)
                for i in range(hypes.num_news_action-1):
                ...
                if(global_step % 10 ==0):
                    ...
                    build_summary(s_batch, s2_batch,
...
def saveAsPb(sess, path):
    ...
    output_graph_def = graph_util.convert_variables_to_constants(
        sess, gd,
        output_node_names=["train/Actor_1/target_Q_net/transpose_2"],
        variable_names_whitelist=None)
...
def main(args):
    ...
    replay_buffer = replay.PER(args.buffer_size)
        ...
        while True:
            ...
            if len(train_files) < 1:
                ...
                for file_num in range(len(train_files)):
                    ...
                    episode_reward_ave=train(sess, userEnv, actor_Q,actor_Q_test, saver, summary_writer,
                                             current_ckpt_path, args,replay_buffer)
                    if(episode_reward_ave < 0.1): #数据有问题，直接删除，不更新模型
                        shutil.rmtree(train_file_tmp)
                        continue
                    ...
                    #online
                    # shell_path = "/data/bd-recommend/lizhenmao/transfer_log_tagShared/updateVideoChannelModel.sh"
                    # os.system(shell_path)

                    #dev
                    shell_path = "/data/bd-recommend/lizhenmao/hbase_transfer_log_tagShared_dev/updateVideoChannelModel.sh"
                    os.system(shell_path)
